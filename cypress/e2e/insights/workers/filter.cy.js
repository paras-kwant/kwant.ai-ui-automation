...
describe('Companies Insights - Filter Functionality', { tags: ["Epic:WorkForce", "Feature:Filter", "Module:Insights-Company"] }, () => {
  let authHeaders = {};
  ...
  it(
	'Insights-Company - validate filters from API except Access Status',
	{
	  tags: [
		"Story:Insights Filter Validation",
		"Severity:normal",
		"API",
		"UI",
    ...
    () => {
      ...
      cy.get('.multi-select-option__head')
        .then($options => {
          ...
          return Cypress._.sampleSize(optionTexts, Math.min(3, optionTexts.length));
        })
        .then(randomTexts => {
          cy.wrap(randomTexts).each(text => {
      ...
      cy.wrap(certOptions).each(option => {
      ...
      cy.wrap(safetyOptions).each(option => {
        cy.contains('.multi-select-option__head', option)
          .scrollIntoView()
          .should('exist');
      });
  ...
  it.only('Insights-Company - Verify Company Name Filter', 
    { tags: ["Story:Insights Filter By Company Name", "Severity:critical", "UI", "Module:Insights-Company"] }, 
    () => {
        ...
        cy.get('body').then(($body) => {
          ...
          if ($rows.length > 0) {
            ...
            cy.wrap($rows).each(($row) => {
              cy.wrap($row)
                .invoke('text')
                .should('include', selectedCompany);
            });
  ...
  it('Insights-Company - Verify Safety Alert filter functionality', { tags: ["Story:Insights Filter By Safety Alert Type", "Severity:critical", "UI", "Module:Insights-Company"] }, () => {
    ...
    cy.contains(workforceSelector.tableColumn, 'Workers On-site').find('.table-header-filter-btn').click();
    cy.get('[placeholder="Min"]').type('1');
    cy.get('[placeholder="Max"]').type('10');
    cy.get('p').contains('Filters:').click();
    cy.wait('@taskTradeFilter').its('response.statusCode').should('eq', 200);
    ...
    cy.get('section [data-testid="table_tr"]').then(($rows) => {
      ...
      uniqueLabels.forEach(label => {
        cy.contains('section [data-testid="table_tr"] .default__label', label).should('exist');
      });
  ...
  it('Insights-Company - Verify Safety Alert filter functionality (Type + Multiple Dates)', { tags: ["Story:Insights Filter Safety Alert Type And Multiple Dates", "Severity:critical", "UI", "Module:Insights-Company"] }, () => {
    ...
    cy.get('section [data-testid="table_tr"] .default__label').then(($labels) => {
      const uniqueLabels = [...new Set([...$labels].map(el => el.innerText.trim()).filter(Boolean))];
      expect(uniqueLabels.length).to.be.greaterThan(0);
      Cypress.env('safetyAlertLabel', uniqueLabels[0]);
    ...
    cy.get('.cell-content').then(($cells) => {
      const uniqueDates = [...new Set([...$cells].map(el => el.innerText.trim().split(' ')[0]).filter(Boolean))];
        ...
        cy.get('body').then(($body) => {
          if ($body.find(workforceSelector.tableRow).length > 0) {
            cy.get(workforceSelector.tableRow).then(($rows) => {
              ...
              expect(found, `Expected company "${companyName}" for date ${date}`).to.be.true;
            });
          } else {
            cy.contains('.empty-body__title', 'No Results Found').should('exist');
          }
  ...
  it('Insights-Company - Verify the Certificate Status', { tags: ["Story:Insights Filter By Certificate Status", "Severity:critical", "UI", "Module:Insights-Company"] }, () => {
    ...
    cy.get('section [data-testid="table_tr"]').first().then(($row) => {
      ...
      cy.get('body').then(($body) => {
        if ($body.find(workforceSelector.tableRow).length > 0) {
          cy.get(workforceSelector.tableRow).find('.personal-info-content__title').then(($titles) => {
  ...
  it('Insights-Company - Verify Certificate Type Filter using document name from row', { tags: ["Story:Insights Filter By Certificate Type", "Severity:critical", "UI", "Module:Insights-Company"] }, () => {
    ...
    cy.get('section [data-testid="table_tr"]').first()
      ...
      .then((docName) => {
        ...
        cy.get('section [placeholder="Search"]').clear().type(documentName);
        cy.contains('.multi-select-option__head', documentName).should('be.visible').click();
        cy.contains('section button p', 'Filter').click();
        cy.get('body').click(0, 0);
        cy.get('.loader-image').should('not.exist');
        cy.wait('@companyTable').its('response.statusCode').should('eq', 200);
        ...
        cy.get(workforceSelector.tableRow).find('.personal-info-content__title').then(($titles) => {
          const names = [...$titles].map(el => el.innerText.trim());
          const found = names.some(name => name.includes(Cypress.env('companyName')));
          expect(found, `Expected company "${Cypress.env('companyName')}" to appear after filtering by document "${documentName}"`).to.be.true;
        });
      });
  });
