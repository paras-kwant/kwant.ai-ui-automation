  ...
  it(
    ...
    () => {
      ...
      cy.get(workforceSelector.tableRow).then(($rows) => {
        const matchedRows = Array.from($rows).filter(
          (row) => Number(Cypress.$(row).find(".site-label").text().trim()) >= 1
        );
            ...
            cy.wait("@getDetail").then((interception) => {
              ...
              cy.get("@authHeaders").then((authHeaders) => {
                cy.request({
                  ...
                  cy.log("PUT Payload:", JSON.stringify(updatedPayload));
                  ...
                  cy.get("footer p")
                    ...
                    .then((text) => {
                      ...
                      if (!match)
                        throw new Error(`Could not parse Remaining Alerts from text: "${text}"`);
                      ...
                      cy.get("footer p")
                        ...
                        .then((updatedText) => {
                          ...
                          if (!updatedMatch)
                            throw new Error(
                              `Could not parse Remaining Alerts after sending alert: "${updatedText}"`
                            );
                        ...
                        return new Cypress.Promise((resolve, reject) => {
                          const check = (remaining) => {
                            cy.request({
                              ...
                              if (res.status === 200 && res.body.messages.length > 0) {
                                const msg = res.body.messages.find(
                                  (m) =>
                                    m.from === expectedFrom &&
                                    m.direction === "inbound" &&
                                    m.body.includes(expectedMessageSnippet)
                                );
                                if (msg) {
                                  resolve(msg);
                                  return;
                              ...
                              if (remaining === 0) {
                                reject(
                                  "Incoming SMS not found in Twilio logs after retries"
                                );
                              } else {
  ...
  it(
    ...
    () => {
	  ...
	  cy.get(workforceSelector.tableRow).then(($rows) => {
		...
		expect(matchedRows.length).to.be.gte(
		  2,
		  "There should be at least two rows with label >=1"
		);
		...
		cy.wrap(selectedWorker)
		  ...
		  .then((text) => {
			...
			// Confirm selected company is visible in the UI
			cy.get(".onsite-selected-container .personal-info-content__title")
			  .contains(companyName)
			  .should("be.visible");
...
cy.get("body").then(($body) => {
	if ($body.find("h1:contains('Send Alert')").length > 0) {
	  ...
	  cy.log("Send Alert modal closed successfully");
	}
  });
	  cy.get("button").contains("Send Alert").click();
