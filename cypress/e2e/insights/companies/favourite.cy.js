...
describe("Insights Company - Favourite Page", { tags: ["Epic:WorkForce", "Feature:Favorites", "Module:Insights-Company"] }, () => {
  ...
  it(" Insight Compay Verify remove company from Favorite", { tags: ["Story:Insights Company Remove From Favorite", "Severity:critical", "UI", "Module:Insights-Company"] }, () => {
  ...
  it("Verify adding company as favourite", { tags: ["Story:Insights Company Add To Favorite", "Severity:critical", "UI", "Module:Insights-Company"] }, () => {
    cy.get(workforceSelector.tableRow).should('be.visible');
    insightsFavouritePage.clickFavouriteButton();
  ...
  it("Verify Insights Company Page Accessibility from Favorites", { tags: ["Story:Insights Company Access From Favorites", "Severity:normal", "UI", "Module:Insights-Company"] }, () => {
    cy.get(workforceSelector.tableRow).should('be.visible');
  ...
  it("Verify Favorite Status Persistence", { tags: ["Story:Insights Company Favorite Status Persistence", "Severity:normal", "UI", "Module:Insights-Company"] }, () => {
    cy.get(workforceSelector.tableRow).should('be.visible');
    insightsFavouritePage.visitInsightsCompaniesPage();
    insightsFavouritePage.assertFavouriteIconVisible();
  });

  it("Verify that the latest company added to favorites is displayed at the top of the favorites list.", { tags: ["Story:Insights Company Favorite List Order", "Severity:normal", "UI", "Module:Insights-Company"] }, () => {
    cy.get(workforceSelector.tableRow).should('be.visible');
