...
() => {
  ...
  it('Should disable buttons when mandatory fields are empty', 
    { tags: ["Story:Mandatory Fields Validation", "Severity:critical", "UI", "Module:Workforce-Worker"] },
  () => {
      addworkerPage.verifyMandatoryButtonsAreDisabled();
  });

  it('Should not allow adding worker without First Name', 
    { tags: ["Story:First Name Mandatory", "Severity:critical", "UI", "Module:Workforce-Worker"] },
  () => {
  ...
  it('Should not allow adding worker without Last Name', 
    { tags: ["Story:Last Name Mandatory", "Severity:critical", "UI", "Module:Workforce-Worker"] },
  () => {
    const workerData = generateWorkerData();
    addworkerPage.uploadProfileImage('profile.png');
  ...
  it('Should not allow adding worker without Company name', 
    { tags: ["Story:Company Mandatory", "Severity:critical", "UI", "Module:Workforce-Worker"] },
  () => {
    const workerData = generateWorkerData();
  ...
  it('Should Validate Retake Picture Functionlity', 
    { tags: ["Story:Retake Picture", "Severity:normal", "UI", "Module:Workforce-Worker"] },
  ()=>{
  ...
  it('Validate the Back button functionlity while clicking picture', 
    { tags: ["Story:Back Button on Camera", "Severity:normal", "UI", "Module:Workforce-Worker"] },
  ()=>{
    addWorkerSelector.profileImageUploadButton().click();
    addWorkerSelector.takeAPictureButton().click();
    cy.get('video.video_viewer').should('be.visible');
  ...
  it('Should validate email format', 
    { tags: ["Story:Email Format Validation", "Severity:normal", "UI", "Module:Workforce-Worker"] },
  () => {
    const workerData = generateWorkerData(); 
    addworkerPage.fillWorkerName(workerData);
    cy.selectRandomOption('input[name="company"]', '.select_item_container [role="button"]', 'company');
  ...
  it('Should restrict worker image upload to PNG, JPG, JPEG', 
    { tags: ["Story:Profile Image File Type Validation", "Severity:normal", "UI", "Module:Workforce-Worker"] },
  () => {
  ...
  it('Should show warning when adding existing worker', 
    { tags: ["Story:Duplicate Worker Validation", "Severity:normal", "UI", "Module:Workforce-Worker"] },
  () => {
  ...
  () => {
    ...
    cy.wait('@addedworker').then(({ response }) => {
      ...
      cy.log('Worker added successfully with ID:', workerId);
    });
  ...
  it('Should add worker with profile picture', 
    { tags: ["Story:Add Worker with Profile Image", "Severity:critical", "UI", "Module:Workforce-Worker"] },
  () => {
    let workerId;
    let requestPayload;
    let authHeaders = {};
  ...
  it('Should capture worker photo via camera', 
    { tags: ["Story:Capture Worker Photo", "Severity:normal", "UI", "Module:Workforce-Worker"] },
  () => {   
    addWorkerSelector.profileImageUploadButton().click();
    addWorkerSelector.takeAPictureButton().click();
  ...
  () => {
    ...
    cy.get(workforceSelector.accessControlPage).click();
    cy.selectRandomOption('[name="projectBeaconSerialNumber"]', '.select_item_container [role="button"]');
    addworkerPage.submitWorker();
