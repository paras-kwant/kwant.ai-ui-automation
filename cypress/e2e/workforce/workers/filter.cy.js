...
import filterPage from "../../../pages/workforce/filter";
const PROJECT_ID = Cypress.env('PROJECT_ID');

describe(
  "Worker Module - Filter",
  { tags: ["Epic:WorkForce", "Feature:Filter", "Module:Workforce-Worker"] },
  () => {
    ...
    it(
      ...
      { tags: ["Story:Filter By Name Header", "Severity:critical", "UI",     ] },
      () => {
    ...
    it(
      'Verify the table header filter exists for applicable table headers',
      { tags: ["Story:Filter Icon Exists On Table Headers", "Severity:normal", "UI", "Module:Workforce-Worker"] },
      () => {
        cy.wait(1000);
          ...
          if (index >= 3) {
            cy.wrap($el).then(($header) => {
              if ($header.find('.table-header-filter-btn').length) {
                cy.wrap($header).find('.table-header-filter-btn svg').should('exist');
              }
            });
          }
        });
    ...
    it(
      ...
      { tags: ["Story:Clear Filter Restores Table State", "Severity:critical", "UI", "Module:Workforce-Worker"] },
      () => {
        ...
        cy.get('[class*="select_item_container"]').within(() => {
          cy.get('label[for^=":r"]').then(($labels) => {
            ...
            cy.wrap(validLabels.eq(randomIndex)).find('input[type="checkbox"]').check({ force: true });
          });
        });
    ...
    it(
      "Verify Multiple Company Selection for Filtering",
      { tags: ["Story:Filter By Multiple Companies", "Severity:critical", "UI", "Module:Workforce-Worker"] },
      () => {
        ...
        cy.get(workforceSelector.tableRow).then(($rows) => {
          ...
          rowsToCheck.each((index, row) => {
            const companyName = Cypress.$(row).find(".table_td:nth-child(6) .cell-content").text().trim();
            if (companyName) companyNamesFromTable.push(companyName);
          });
          ...
          cy.wrap(uniqueCompanies).each((selectedCompany) => {
            ...
            cy.get('[class*="select_item_container"]').within(() => {
              cy.get('label[for^=":r"]').each(($label) => {
                const companyName = Cypress.$($label).find('span[type="onDropdown"]').last().text().trim();
                if (companyName === selectedCompany) {
                  cy.wrap($label).find('input[type="checkbox"]').check({ force: true });
                  return false;
                }
              });
            });
              ...
              cy.get(workforceSelector.tableRow).each(($row) => {
                cy.wrap($row).within(() => {
                  cy.get(".table_td:nth-child(6) .cell-content").then(($cell) => {
                    ...
                    expect(uniqueCompanies.includes(companyName), `Company "${companyName}" should be one of: ${uniqueCompanies.join(", ")}`).to.be.true;
                  });
                });
              });
            } else {
    ...
    it(
      "Verify combined filters (Name + Company + Job Title + Site Status)",
      { tags: ["Story:Filter Combined Name Company JobTitle SiteStatus", "Severity:normal", "UI", "Module:Workforce-Worker"] },
      () => {
        ...
        cy.wait("@workersApi").then((interception) => {
          const workers = interception.response.body.projectWorkerDTOS;
          const randomName = Cypress._.sample(workers.map(w => w.firstName).filter(Boolean));
          cy.log(`Selected Name: ${randomName}`);

          cy.contains(workforceSelector.tableColumn, "Name").find(".table-header-filter-btn").click({ force: true });
        ...
        cy.get('[class*="select_item_container"]').within(() => {
          cy.get('label[for^=":r"]').then(($labels) => {
            ...
            const company = $randomLabel.find('span[type="onDropdown"]').last().text().trim();
        ...
        cy.contains(workforceSelector.tableColumn, "Job Title").find(".table-header-filter-btn").click({ force: true });
        cy.get('[placeholder="Search"]').last().type("worker");
        cy.contains("Filters:").click();
        cy.wrap("worker").as("selectedJobTitle");

        cy.contains(workforceSelector.tableColumn, "Site Status").find(".table-header-filter-btn").click({ force: true });
        ...
        cy.get('[class*="select_item_container"]').within(() => {
          cy.get('label[for^=":r"]').then(($labels) => {
            ...
            const randomIndex = Cypress._.random(0, valid.length - 1);
            const $randomLabel = valid.eq(randomIndex);
            const status = $randomLabel.find('span[type="onDropdown"]').last().text().trim();
        ...
        cy.then(function () {
          cy.get("body").then(($body) => {
            if ($body.find(workforceSelector.tableRow).length > 0) {
              cy.get(`${workforceSelector.tableRow} .personal-info-content__title`).contains(this.selectedName).should("be.visible");
              cy.get(".table_td:nth-child(6) .cell-content").contains(this.selectedCompany).should("be.visible");
              cy.get(".table_td:nth-child(7) .cell-content").contains(this.selectedJobTitle).should("be.visible");
              if (this.selectedStatus.toLowerCase() === "flagged") {
                cy.get(".dot-container__status-label").should("contain.text", "Red");
              } else {
                cy.get(".dot-container__status-label").contains(this.selectedStatus).should("be.visible");
              }
            } else {
    ...
    it(
      "Verifies the Company filter works correctly",
      { tags: ["Story:Filter By Company", "Severity:critical", "UI", "Module:Workforce-Worker"] },
      () => {
        filterPage.applyFilter("Company");
        cy.get('@selectedFilterValue').then((selectedValue) => {
          filterPage.verifyFilteredRows(workforceSelector.generalDetailsPage, 'Company', selectedValue);
        });
      }
    );
    ...
    it(
      ...
      { tags: ["Story:Filter Empty Results Shows No Results", "Severity:critical", "UI",     ] },
      () => {
        cy.get(".table-header-filter-btn").eq(0).click();
        cy.get(workforceSelector.searchInput).eq(1).type("NonExistentWorkerName");
        cy.get("p").contains("Filters:").click();
        cy.wait(1000);
        cy.get(".empty-body").contains("No Results Found");
      }
    );

    it(
      "Verify Name Filter Supports Case Insensitivity (Uppercase, Lowercase, Mixed Case)",
      { tags: ["Story:Filter Name Case Insensitive", "Severity:critical", "UI", "Module:Workforce-Worker"] },
      () => {
        cy.intercept("POST", "/api/filterProjectWorker*").as("workersApi");
        cy.reload();
        ...
        cy.wait("@workersApi").then((interception) => {
          ...
          const mixed = name.split("").map((c) => (Math.random() > 0.5 ? c.toUpperCase() : c.toLowerCase())).join("");

          const runSearch = (searchValue) => {
            cy.contains(workforceSelector.tableColumn, 'Name').find('.table-header-filter-btn').click();
            cy.get(workforceSelector.searchInput).eq(1).clear().type(searchValue);
            cy.get("p").contains("Filters:").click();
            cy.wait(1000);
    ...
    it(
      "Validate removing filter one after another",
      { tags: ["Story:Remove Filters One By One", "Severity:critical", "UI", "Module:Workforce-Worker"] },
      () => {
    ...
    it(
      "Verify Filter of the different column for Job (Title)",
      { tags: ["Story:Filter By Job Title", "Severity:critical", "UI", "Module:Workforce-Worker"] },
      () => {
        cy.get(".table-header-filter-btn").eq(2).click();
        ...
        cy.get("body").then(($body) => {
          if ($body.find(workforceSelector.tableRow).length > 0) {
            cy.get(".table_td:nth-child(7) .cell-content").then(($cells) => {
              const hasMatch = [...$cells].some((cell) => cell.innerText.toLowerCase().includes("tech"));
    ...
    it(
      "Verify filter of the Site Status column",
      { tags: ["Story:Filter By Site Status", "Severity:critical", "UI", "Module:Workforce-Worker"] },
      () => {
        cy.contains(workforceSelector.tableColumn, "Site Status").find(".table-header-filter-btn").click({ force: true });
        ...
        cy.get('[class*="select_item_container"]').within(() => {
          cy.get('label[for^=":r"]').then(($labels) => {
            ...
            expect(validLabels.length, "Valid Site Status options").to.be.greaterThan(0);
            const randomIndex = Cypress._.random(0, validLabels.length - 1);
            const $randomLabel = validLabels.eq(randomIndex);
            const statusName = $randomLabel.find('span[type="onDropdown"]').last().text().trim();
          ...
          cy.get("body").then(($body) => {
            if ($body.find(workforceSelector.tableRow).length > 0) {
              if (statusName.toLowerCase() === "flagged") {
                cy.get(".dot-container__status-label").should("contain.text", "Red").and("be.visible");
              } else {
                cy.contains(".dot-container__status-label", statusName).scrollIntoView().should("be.visible");
              }
            } else {
              cy.get(".empty-body").contains("No Results Found");
            }
    ...
    it(
      "Verify filter Interaction with Search Bar Filter",
      { tags: ["Story:Filter Interaction With Search Bar", "Severity:normal", "UI", "Module:Workforce-Worker"] },
      () => {
        cy.intercept("POST", "/api/filterProjectWorker*").as("workersApi");
        cy.reload();
        ...
        cy.wait("@workersApi").then((interception) => {
          ...
          cy.get(workforceSelector.tableRow).eq(0).find(".cell-content").eq(2).invoke("text").then((jobTitleText) => {
            ...
            cy.wait(1000);
            cy.get(`${workforceSelector.tableRow} .personal-info-content__title`).eq(0).should("contain.text", randomNames[0]);
            cy.get(workforceSelector.tableRow).eq(0).find(".cell-content").eq(2).should("contain.text", jobTitle);
          });
        });
      }
    );
    ...
    it(
      ...
      { tags: ["Story:Filter Funnel Icon Visible On Headers", "Severity:normal", "UI",     ] },
      () => {
        cy.get(workforceSelector.tableColumn).each(($el, index) => {
          if (index >= 3) {
            cy.wrap($el).then(($header) => {
              if ($header.find('.table-header-filter-btn').length) {
                cy.wrap($header).find('.table-header-filter-btn').should('exist').find('svg').should('exist');
              }
            });
          }
        });
    ...
    it(
      "Verify all selected options are visible on hover over pills",
      { tags: ["Story:Filter Pill Hover Shows Selected Options", "Severity:normal", "UI", "Module:Workforce-Worker"] },
      () => {
        cy.contains(workforceSelector.tableColumn, "Company").find(".table-header-filter-btn").click({ force: true });
        ...
        cy.get('[class*="select_item_container"]').within(() => {
          cy.get('label[for^=":r"]').then(($labels) => {
            ...
            const randomIndex = Cypress._.random(0, validLabels.length - 1);
            const $randomLabel = validLabels.eq(randomIndex);
            const companyName = $randomLabel.find('span[type="onDropdown"]').last().text().trim();
        ...
        cy.contains("Filters:").click();
        cy.get(".filter-tag").should("be.visible").and("contain.text", "Company Name");

        cy.get("@selectedCompany").then((companyName) => {
          cy.get(".filter-tag").realHover();
          cy.wait(300);
          cy.contains(".label.default__label", companyName).should("be.visible");
        });
      }
    );
    ...
    it(
      ...
      { tags: ["Story:Filter Red Dot Indicator On Active Filter", "Severity:normal", "UI",     ] },
      () => {
        cy.contains(workforceSelector.tableColumn, "Company").find(".table-header-filter-btn").click({ force: true });
        ...
        cy.contains(workforceSelector.tableColumn, "Company").find(".table-header-filter-btn").within(() => {
    ...
    it(
      "Verifies the Phone filter works correctly",
      { tags: ["Story:Filter By Phone Number", "Severity:critical", "UI", "Module:Workforce-Worker"] },
      () => {
        cy.get(workforceSelector.tableColumn).contains('Phone').parent().find('svg').click({ force: true });
    ...
    it(
      "Verifies the Last Seen Location filter works correctly for all rows",
      { tags: ["Story:Filter By Last Seen Location", "Severity:critical", "UI", "Module:Workforce-Worker"] },
      () => {
        filterPage.applyFilter("Device");
        cy.get('@selectedFilterValue').then((selectedValue) => {
          filterPage.verifyFilteredRows(workforceSelector.accessControlPage, 'Last Seen On', selectedValue);
        });
      }
    );

    it(
      "Verifies the Race filter works correctly for all rows",
      { tags: ["Story:Filter By Race", "Severity:critical", "UI", "Module:Workforce-Worker"] },
      () => {
        filterPage.applyFilter("Race");
        cy.get('@selectedFilterValue').then((selectedValue) => {
          filterPage.verifyFilteredRows(workforceSelector.personalDetailsPage, 'Race', selectedValue);
        });
      }
    );

    it(
      "Verifies the Device filter works correctly",
      { tags: ["Story:Filter By Device", "Severity:critical", "UI", "Module:Workforce-Worker"] },
      () => {
        filterPage.applyFilter("Device");
        cy.get('@selectedFilterValue').then((selectedValue) => {
          filterPage.verifyFilteredRows(workforceSelector.accessControlPage, 'Device', selectedValue);
        });
      }
    );

    it(
      "Verifies the Sex filter works correctly",
      { tags: ["Story:Filter By Sex", "Severity:critical", "UI", "Module:Workforce-Worker"] },
      () => {
        filterPage.applyFilter("Sex");
        cy.get('@selectedFilterValue').then((selectedValue) => {
          filterPage.verifyFilteredRows(workforceSelector.personalDetailsPage, 'Sex', selectedValue);
        });
      }
    );

    it(
      "Verifies the Ethnicity filter works correctly for all rows",
      { tags: ["Story:Filter By Ethnicity", "Severity:critical", "UI", "Module:Workforce-Worker"] },
      () => {
        filterPage.applyFilter("Ethnicity");
        cy.get('@selectedFilterValue').then((selectedValue) => {
          cy.log(`Verifying all rows have Ethnicity: ${selectedValue}`);
          filterPage.verifyFilteredRows(workforceSelector.personalDetailsPage, 'Ethnicity', selectedValue);
        });
      }
    );

    it(
      "Verifies the Safety Alert filter works correctly for all rows",
      { tags: ["Story:Filter By Safety Alert", "Severity:critical", "UI", "Module:Workforce-Worker"] },
      () => {
        cy.contains(workforceSelector.tableColumn, 'Safety Alert').find('.table-header-filter-btn').click({ force: true });
        ...
        cy.get('[class*="select_item_container"]').within(() => {
          cy.get('label[for^=":r"]').then(($labels) => {
            ...
            if (validLabels.length === 0) {
              ...
              selectedAlert = $first.find('span[type="onDropdown"]').last().text().trim();
              cy.wrap($first).find('input[type="checkbox"]').check({ force: true });
            } else {
              const randomIndex = Cypress._.random(0, validLabels.length - 1);
              const $randomLabel = validLabels.eq(randomIndex);
              selectedAlert = $randomLabel.find('span[type="onDropdown"]').last().text().trim();
              cy.wrap($randomLabel).find('input[type="checkbox"]').check({ force: true });
            }
    ...
    it(
      "Verifies the Crew filter works correctly for all rows",
      { tags: ["Story:Filter By Crew", "Severity:critical", "UI", "Module:Workforce-Worker"] },
      () => {
        filterPage.applyFilter("Crew");
        cy.get('@selectedFilterValue').then((selectedValue) => {
          cy.log(`Verifying all rows have Crew: ${selectedValue}`);
          filterPage.verifyFilteredRows(workforceSelector.jobDetailsPage, 'Crew', selectedValue);
        });
      }
    );

    it(
      "Validates Boolean filter results based on checkbox state",
      { tags: ["Story:Filter By Boolean Field", "Severity:critical", "UI", "Module:Workforce-Worker"] },
      () => {
        cy.contains(workforceSelector.tableColumn, "Boolean test").find(".table-header-filter-btn").click({ force: true });
        ...
        cy.get('[class*="select_item_container"]').within(() => {
          cy.get('label[for^=":r"]').then(($labels) => {
            ...
            const $selectedLabel = validLabels.length > 0
              ? validLabels.eq(Cypress._.random(0, validLabels.length - 1))
              : $labels.eq(0);

            const selectedValue = $selectedLabel.find('span[type="onDropdown"]').last().text().trim();
            cy.wrap($selectedLabel).find('input[type="checkbox"]').check({ force: true });
              ...
              cy.get(workforceSelector.tableRow).each(($row) => {
                cy.wrap($row).within(() => {
                  if (selectedValue === "True") {
                    cy.get('.exact-toggle-switch [type="checkbox"]').should("exist").and("be.checked");
                  } else if (selectedValue === "False") {
                    cy.get('.exact-toggle-switch [type="checkbox"]').should("exist").and("not.be.checked");
                  }
                });
              });
            } else {
