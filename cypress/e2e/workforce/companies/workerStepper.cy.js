...
cy.wait(200)
  ...
  it('WorkForce-Company - Add a flagged worker and verify total flagged count increases', { tags: ["Story:Add Flagged Worker Increases Count", "Severity:critical", "UI", "@smoke"] }, () => {
    ...
    cy.get('body').then(($body) => {
      if ($body.find('p:contains("Flagged Workers On-site")').length > 0) {
        cy.contains('p', 'Flagged Workers On-site')
          .parent()
          .parent()
          .parent()
        ...
        cy.log('Flagged Workers On-site card not present — no flagged workers yet, setting initial count to 0');
        cy.wrap(0).as('initialFlaggedCount');
      }
    });
