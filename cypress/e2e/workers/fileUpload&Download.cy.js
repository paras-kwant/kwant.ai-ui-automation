...
describe(
  ...
  { tags: ["Epic:WorkForce", "Feature:FileUpload", "Module:Workforce-Worker"] },
  () => {
    ...
    it(
      ...
      { tags: ["Story:Upload Valid CSV Adds Worker", "Severity:critical", "UI", "Module:Workforce-Worker"] },
      () => {
    ...
    it(
      ...
      { tags: ["Story:Download Worker CSV Template", "Severity:critical", "UI", "Module:Workforce-Worker"] },
      () => {
    ...
    it(
      'Should verify downloaded worker CSV content matches UI worker list',
      { tags: ["Story:Downloaded CSV Matches UI Worker List", "Severity:critical", "UI", "Module:Workforce-Worker"] },
      () => {
    ...
    it(
      'Should verify downloaded CSV contains only the selected worker',
      { tags: ["Story:Downloaded CSV Contains Only Selected Worker", "Severity:critical", "UI", "Module:Workforce-Worker"] },
      () => {
        const FILE_NAME = 'Ontarget-Employee-Report.csv';
        const DOWNLOADS_FOLDER = Cypress.config("downloadsFolder");
        const FILE_PATH = path.join(DOWNLOADS_FOLDER, FILE_NAME);
    ...
    it(
      ...
      { tags: ["Story:Upload CSV Via Drag And Drop", "Severity:critical", "UI", "Module:Workforce-Worker"] },
      () => {
        cy.get(workforceSelector.overflowMenu).click();
        ...
        cy.fixture('testdata/employeeUpload.csv', 'base64').then(fileContent => {
          cy.contains('button', 'Choose File').attachFile(
            { fileContent, fileName: 'employeeUpload.csv', mimeType: 'text/csv', encoding: 'base64' },
            { subjectType: 'drag-n-drop', force: true }
          );
        });

        cy.get('[type="info"]').should('be.visible').and('contain.text', '1 worker(s) will be added.');
    ...
    it(
      'Should display message when uploading a CSV with no worker data',
      { tags: ["Story:Upload Empty CSV Shows No Content Message", "Severity:critical", "UI", "Module:Workforce-Worker"] },
      () => {
        cy.get(workforceSelector.overflowMenu).click();
        cy.get('.dropdown-option').contains('Upload').click();
        ...
        cy.fixture('backup.csv', 'base64').then(fileContent => {
          cy.contains('button', 'Choose File').attachFile(
            { fileContent, fileName: 'EmptyCsv.csv', mimeType: 'text/csv', encoding: 'base64' },
            { subjectType: 'drag-n-drop', force: true }
          );
        });

        cy.get('[type="info"]').should('be.visible').and('contain.text', 'No content to upload.');
      }
    );

    it(
      'Should show error when uploading non-CSV or non-Excel file',
      { tags: ["Story:Upload Invalid File Type Shows Error", "Severity:critical", "UI", "Module:Workforce-Worker"] },
      () => {
        ...
        cy.fixture('demo.pdf', 'base64').then(fileContent => {
          cy.contains('button', 'Choose File').attachFile(
            { fileContent, fileName: 'demo.pdf', mimeType: 'pdf', encoding: 'base64' },
            { subjectType: 'drag-n-drop', force: true }
          );
        });
    ...
    it(
      ...
      { tags: ["Story:Upload CSV Missing Company Name Validation", "Severity:critical", "UI", "Module:Workforce-Worker"] },
      () => {
        cy.get(workforceSelector.overflowMenu).click();
        cy.get('.dropdown-option').contains('Upload').click();
        ...
        cy.fixture('noCompany.csv', 'base64').then(fileContent => {
          cy.contains('button', 'Choose File').attachFile(
            { fileContent, fileName: 'noCompany.csv', mimeType: 'csv', encoding: 'base64' },
            { subjectType: 'drag-n-drop', force: true }
          );
        });
    ...
    it(
      ...
      { tags: ["Story:Upload CSV Missing First Name Validation", "Severity:critical", "UI", "Module:Workforce-Worker"] },
      () => {
        cy.get(workforceSelector.overflowMenu).click();
        cy.get('.dropdown-option').contains('Upload').click();
        ...
        cy.fixture('testdata/noFirstName.csv', 'base64').then(fileContent => {
          cy.contains('button', 'Choose File').attachFile(
            { fileContent, fileName: 'noFirstName.csv', mimeType: 'csv', encoding: 'base64' },
            { subjectType: 'drag-n-drop', force: true }
          );
        });
    ...
    it(
      ...
      { tags: ["Story:Upload CSV Missing Last Name Validation", "Severity:critical", "UI", "Module:Workforce-Worker"] },
      () => {
        cy.get(workforceSelector.overflowMenu).click();
        cy.get('.dropdown-option').contains('Upload').click();
        ...
        cy.fixture('testdata/noLastName.csv', 'base64').then(fileContent => {
          cy.contains('button', 'Choose File').attachFile(
            { fileContent, fileName: 'noLastName.csv', mimeType: 'csv', encoding: 'base64' },
            { subjectType: 'drag-n-drop', force: true }
          );
        });
    ...
    it(
      'Should show duplicate worker validation message for duplicate CSV data',
      { tags: ["Story:Upload Duplicate CSV Shows Warning", "Severity:critical", "UI", "Module:Workforce-Worker"] },
      () => {
        cy.wait(2000);
        cy.get(workforceSelector.overflowMenu).click();
        cy.get('.dropdown-option').contains('Upload').click();
        ...
        cy.fixture('testdata/duplicateWorker.csv', 'base64').then(fileContent => {
          cy.contains('button', 'Choose File').attachFile(
            { fileContent, fileName: 'duplicateWorker.csv', mimeType: 'csv', encoding: 'base64' },
            { subjectType: 'drag-n-drop', force: true }
          );
        });
    ...
    it(
      'Should detect and show invalid phone number message from CSV upload',
      { tags: ["Story:Upload CSV Invalid Phone Number Validation", "Severity:critical", "UI", "Module:Workforce-Worker"] },
      () => {
        cy.get(workforceSelector.overflowMenu).click();
        cy.get('.dropdown-option').contains('Upload').click();
        ...
        cy.fixture('testdata/invalidPhoneNumber.csv', 'base64').then(fileContent => {
          cy.contains('button', 'Choose File').attachFile(
            { fileContent, fileName: 'invalidPhoneNumber.csv', mimeType: 'csv', encoding: 'base64' },
            { subjectType: 'drag-n-drop', force: true }
          );
        });

        cy.get('[type="info"]').should('be.visible').and('contain.text', '1 Invalid Phone Number');
      }
    );

    it(
      'Should upload CSV with invalid fields and validate profile fallback values',
      { tags: ["Story:Upload CSV Invalid Fields Show Fallback Values", "Severity:critical", "UI", "Module:Workforce-Worker"] },
      () => {
        cy.get(workforceSelector.overflowMenu).click();
        cy.get('.dropdown-option').contains('Upload').click();
        ...
        cy.fixture('testdata/invalidFields.csv', 'base64').then(fileContent => {
          cy.contains('button', 'Choose File').attachFile(
            { fileContent, fileName: 'invalidFields.csv', mimeType: 'csv', encoding: 'base64' },
            { subjectType: 'drag-n-drop', force: true }
          );
        });
    ...
    it(
      'Should show - while the project code is invalid',
      { tags: ["Story:Upload CSV Invalid Project Code Shows Dash", "Severity:normal", "UI", "Module:Workforce-Worker"] },
      () => {
        cy.wait(2000);
        cy.get(workforceSelector.overflowMenu).click();
        cy.get('.dropdown-option').contains('Upload').click();
        ...
        cy.fixture('testdata/projectcode.csv', 'base64').then(fileContent => {
          cy.contains('button', 'Choose File').attachFile(
            { fileContent, fileName: 'projectcode.csv', mimeType: 'csv', encoding: 'base64' },
            { subjectType: 'drag-n-drop', force: true }
          );
        });
    ...
    it(
      'submiting device with instance id and without mac id',
      { tags: ["Story:Upload Device Without MAC ID Fails Validation", "Severity:critical", "UI", "Module:Workforce-Worker"] },
      () => {
        cy.wait(2000);
        cy.get(workforceSelector.overflowMenu).click();
        cy.get('.dropdown-option').contains('Upload').click();
        ...
        cy.fixture('testdata/nomacid.csv', 'base64').then(fileContent => {
          cy.contains('button', 'Choose File').attachFile(
            { fileContent, fileName: 'nomacid.csv', mimeType: 'csv', encoding: 'base64' },
            { subjectType: 'drag-n-drop', force: true }
          );
        });
    ...
    it(
      'Uploading device without first name, last name and company',
      { tags: ["Story:Upload Device Without Worker Info Imports Beacon", "Severity:critical", "UI", "Module:Workforce-Worker"] },
      () => {
    ...
    it(
      'Uploading device with worker so that it gets assigned',
      { tags: ["Story:Upload Device With Worker Gets Assigned", "Severity:critical", "UI", "Module:Workforce-Worker"] },
      () => {
        ...
        cy.readFile('cypress/fixtures/testdata/validintancemacid.csv').then((content) => {
          ...
          cy.writeFile('cypress/fixtures/testdata/validintancemacid.csv', `${lines[0]}\n${data.join(',')}`);
        });
    ...
    it(
      'Uploading device with already exist ids',
      { tags: ["Story:Upload Device With Existing IDs Shows Warning", "Severity:critical", "UI", "Module:Workforce-Worker"] },
      () => {
        cy.get(workforceSelector.overflowMenu).should('be.visible').click();
        cy.get('.dropdown-option').contains('Upload').click();
    ...
    it(
      'Uploading device with EMPTY instance id should fail validation',
      { tags: ["Story:Upload Device Empty Instance ID Fails Validation", "Severity:critical", "UI", "Module:Workforce-Worker"] },
      () => {
        const instanceId = '';
        ...
        cy.get('[type="info"]').contains('1 Beacon will be imported.').should('not.exist');
        cy.wait(3000);
    ...
    it(
      'Should upload CSV, verify worker details, and delete the entry',
      { tags: ["Story:Upload Full CSV Verify Worker Details And Delete", "Severity:critical", "UI", "Module:Workforce-Worker"] },
      () => {
        cy.get(workforceSelector.overflowMenu).click();
        cy.get('.dropdown-option').contains('Upload').click();
        ...
        cy.fixture('testdata/fulldata.csv', 'base64').then(fileContent => {
          cy.contains('button', 'Choose File').attachFile(
            { fileContent, fileName: 'fulldata.csv', mimeType: 'csv', encoding: 'base64' },
            { subjectType: 'drag-n-drop', force: true }
          );
        });
