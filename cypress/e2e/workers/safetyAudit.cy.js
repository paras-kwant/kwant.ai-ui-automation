  ...
  () => {
      ...
      () => {
        ...
        cy.get(`section ${workforceSelector.tableRow}`)
          ...
          .then((resolvedAlert) => {
            ...
            cy.get('body').then(($body) => {
              if ($body.find(`section ${workforceSelector.tableRow}`).length > 0) {
                cy.get(`section ${workforceSelector.tableRow}`).find('.cell-content').then(($cells) => {
                  const texts = $cells.toArray().map(el => el.innerText.trim());
                  const match = texts.filter(text => text === trimmedAlert);
    ...
    it(
      ...
      () => {
        ...
        const editComment = () => {
          ...
          cy.get('.comment-item-body__content').eq(0).should('contain.text', randomEditedComment);
        };
    ...
    it(
      ...
      () => {
        ...
        cy.get('.comment-item-body__content').first().invoke('text').then((text) => {
    ...
    it(
      ...
      () => {
        cy.get(workforceSelector.tableRow).eq(1).within(() => {
          cy.get('.personal-info-content__title').invoke('text').then((name) => {
        ...
        cy.get(workforceSelector.tableRow).eq(1).within(() => {
          ...
          cy.get('.selected-container .default__label').invoke('text').then((selectedText) => {
