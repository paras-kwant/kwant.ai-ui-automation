...
() => {
  ...
  it("Should display red tag when an invalid picture is uploaded", 
    { tags: ["Story:Invalid Profile Image", "Severity:critical", "UI", "Module:Workforce-Worker"] }, 
  () => {
    ...
    cy.get(workforceSelector.toastMessage)
      .contains('Successfully updated worker')
      .should('be.visible');
    ...
    cy.get(workforceSelector.tableRow).eq(0).within(() => {
      cy.get('.table_td').find('.tag.default.error').should('exist');
    });
  });

  it('Should display green tag when a valid profile image is uploaded', 
    { tags: ["Story:Valid Profile Image", "Severity:critical", "UI", "Module:Workforce-Worker"] }, 
  () => {
    cy.get(workforceSelector.searchInput).clear().type('aaron ashton');
    cy.get(workforceSelector.tableRow).eq(0).should('be.visible').click({ force: true });
    ...
    cy.get(workforceSelector.tableRow).eq(0).within(() => {
      cy.get('.table_td').find('.tag.default.success').should('exist');
    });
  });

  it("Should validate NFC tag color for each worker row based on NFC value", 
    { tags: ["Story:NFC Tag Color Validation", "Severity:normal", "UI", "Module:Workforce-Worker"] }, 
  () => {
    cy.wait(2000);
    cy.get(workforceSelector.tableRow).each(($row) => {
      ...
      cy.getWorkerField('NFC').invoke('text').then((nfcText) => {
        const value = nfcText.trim();
        cy.log(`NFC Text: ${value || '(empty)'}`);

        if (value === '-' || value === '') {
          cy.wrap($row).find('.tag.default.error').should('exist');
        } else {
          cy.wrap($row).find('.tag.default.success').should('exist');
        }
      });
  ...
  it("Should show red device tag when the device is not assigned to the worker", 
    { tags: ["Story:Device Tag Validation", "Severity:critical", "UI", "Module:Workforce-Worker"] }, 
  () => {
    cy.get(workforceSelector.searchInput).clear().type('aaron ashton');
    cy.get(workforceSelector.tableRow).eq(0).should('be.visible').click({ force: true });
    cy.get(workforceSelector.accessControlPage).click();
