...
describe("WorkForce Companies Module - Favourite Page", { tags: ["Epic:WorkForce", "Feature:Favorites", "Module:WorkForce-Company"] }, () => {
	...
	it("Verify remove company from Favorite", { tags: ["Story:Remove Favorite Company", "Severity:critical", "UI", "Module:WorkForce-Company"] }, () => {
	...
	it("Verify adding company as favourite", { tags: ["Story:Add Favorite Company", "Severity:critical", "UI", "Module:WorkForce-Company"] }, () => {
		cy.get('.top-nav-left-section [role="button"]').click();
		cy.get(workforceSelector.toastMessage).contains('Added to favorite');
		cy.get('.top-nav-left-section [role="button"] [fill="#FACC15"]').should('be.visible');
	});

	it('Verify Company Page Accessibility from Favorites', { tags: ["Story:Access Favorite Company Page", "Severity:normal", "UI", "Module:WorkForce-Company"] }, () => {
	...
	it('Verify Favorite Status Persistence', { tags: ["Story:Favorite Status Persistence", "Severity:normal", "UI", "Module:WorkForce-Company"] }, () => {
		cy.visit(`/projects/${Cypress.env('PROJECT_ID')}/companies`);
		cy.get('.top-nav-left-section [role="button"] [fill="#FACC15"]').should('be.visible');
	});

	it('Verify that the latest company added to favorites is displayed at the top of the favorites list.', { tags: ["Story:Favorite List Order", "Severity:normal", "UI", "Module:WorkForce-Company"] }, () => {
