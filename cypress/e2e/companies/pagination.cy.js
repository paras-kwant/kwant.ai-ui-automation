  ...
  it('Verify Next and previous button navigates to respective page', () => {
    ...
    cy.getTotalWorkers().then((totalValue) => {
      ...
      if (totalPages > 1) {
        ...
        cy.get('.workforce-footer button svg').eq(0).closest('button').should('be.disabled');
        cy.get('.workforce-footer button svg').eq(1).closest('button').should('not.be.disabled');
		cy.get('.workforce-footer button svg').eq(1).closest('button').click();
        cy.wait(1000);

		cy.get('.workforce-footer button svg').eq(0).closest('button').should('not.be.disabled');
        cy.get('.workforce-footer button svg').eq(1).closest('button').should('not.be.disabled');

        cy.get('.workforce-footer button svg').eq(0).closest('button').click()
        cy.wait(1000);
        ...
        cy.get('.workforce-footer button svg').eq(0).closest('button').should('be.disabled');
		cy.get('.workforce-footer button svg').eq(1).closest('button').should('be.disabled');      }
    });
...
it('Verify LAST page disables Next button', () => {
  ...
  cy.getTotalWorkers().then((totalValue) => {
    ...
    if (totalPages > 1) {
      cy.log(`Total workers: ${totalValue}, Total pages: ${totalPages}`);
	  cy.get('button.sc-ktJbId.kclJpO')
	  .last()
	  .should('be.visible')
	  .click();
		  cy.get('.workforce-footer button svg').eq(1).closest('button').should('be.disabled');
